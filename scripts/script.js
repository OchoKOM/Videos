...
if (inputBox != null) {
    inputBox.onkeydown = e => {
        requestAnimationFrame(() => {
            ...
            (suggestBox.innerHTML === "") ? suggestBox.classList.add("hide") : suggestBox.classList.remove("hide");
        });
    }
    inputBox.onkeyup = e => {
        if (suggestBox != null) {
            ...
            if (location.href.includes("edit_video/") ||
                ...
                xhr.open('GET', '../searchAction?search');
            } else {
                xhr.open('GET', 'searchAction?search');
            }
...
const mini_players = document.querySelectorAll('.thumbNails:not(.user-profile)'),
...
mini_players.forEach(mini_player => {
    ...
    let source = `kom${mini_player.getAttribute('data-mini')}`;

    let mini_poster = mini_player.querySelector("[data-thumbnail]"),
    link = `watch?v=${mini_player.getAttribute('data-mini')}`,
    mini_video = mini_player.querySelector('video'),
    button = mini_player.querySelector('button:not(.sound)'),
    volume = mini_player.querySelector('.sound'),
    progressBar = mini_player.querySelector('.progress-bar'),
    progress = mini_player.querySelector('.progress');
    ...
    mini_poster.addEventListener('mouseenter', () => {
        ...
        mini_video.autoplay = true;
        mini_video.src = source;
...
function volumeChangeIcon(e) {
    if (e.classList.contains('on')) {
        ...
        mini_players.forEach(mini_player => {
            ...
            video.volume = getVolume ? getVolume : 1;
        })
    } else {
        volumeMuted.forEach(item => {
            item.style.display = "block";
