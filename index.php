...
$fetchVideos = $bdd->prepare('SELECT video.id AS video_id, profil.id AS profil_id, pseudo, avatar, titre, poster, vues, date FROM video JOIN profil ON video.id_auteur = profil.id ORDER BY date DESC LIMIT 100');
$fetchVideos->execute();
?>
<!DOCTYPE html>
<html lang="fr">
...
if (isset($_GET['q']) && !empty($_GET['q'])) {
    $userSearch = $_GET['q'];
    ?>
    <title> <?=$userSearch;?> - Recherche</title>
    <?php 
}else{
    ?>
    <title>OchoVid</title>
        ...
        <ul>
            ...
            <li class="search">
                <form action="search" method="get" >
                    ...
                    <div class="inputbox">
                        <input type="search" placeholder="Chercher" name="q"autocomplete="false" id="search" required>
                    </div>
        ...
        <ul>
            <li>
                <a href="./" class="btn-icon">
                    <svg class="play-icon" viewBox="0 0 48 48" >
                        <path fill="currentColor" d="M8 42V18L24 6l16 12v24H28V28h-8v14Z"/>
                    </svg>
                    <span class="btn-text">
                        Accueil
                    </span>
...
<?php }?> 
    <div class="grid <?php if (($devicecheck == 1)) { ?>mobile<?php } ?>">
        <?php
                ...
                <?php
                    ...
                    while($row = $fetchVideos->fetch()){
                        ...
                        ?>
                    <div class="links">
                        <div class="thumbNails" data-mini="<?=$videoId?>">
                            <a>
                                <div class="mini-player" >
                                    <video autoplay></video>
                                    <img src="<?=$poster;?>" data-thumbnail="<?=$videoId?>" alt="Thumbnail of <?=$name;?>" onclick="location.replace('watch?v=<?=$videoId?>')">
                                </div>
                                <div class="buttons">
                            ...
                            <div class="vidInf">
                                ...
                                <div class="textInf">
                                    <a href="watch?v=<?=$videoId?>" title="<?=$name;?>">
                                        <h4 class="vidName<?=$videoId?>"><?=$name;?></h4>
                                    </a>
                                    <div class="auteur">
                                        <a href="<?=$userPseudo;?>" title="<?=$userPseudo;?>">
                                            <?=$userPseudo;?>  
