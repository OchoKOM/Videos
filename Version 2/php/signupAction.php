    ...
    if (!empty($imgName)) {
        ...
        if ($imgSize < $tailleMax) {
            if (in_array($extentionUpload, $extentionsValides)) {
                ...
                $chemin = "img/" . uniqid() . '_' . $_SESSION['id'] . "." . $extentionUpload;
                $deplacer = move_uploaded_file($_FILES['img']['tmp_name'], $chemin);

                if ($deplacer) {

                    $updateAvatar = $bdd->prepare('UPDATE profil SET avatar= :avatar WHERE id= :id');
                    $updateAvatar->execute(
                        array(
                            'avatar' => 'accounts/'.$chemin,
                            'id' => $_SESSION['id']
                        )
                    );
                    // Récupérer les informations de l'utilisateur 
