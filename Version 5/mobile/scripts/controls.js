...
var launchFullScreen = function (el) {
  if (el.requestFullscreen) {
    ...
    el.mozRequestFullScreen().then(function() {
      // Votre code ici
    }).catch(function() {
      // Votre code ici
    });;
  } else if (el.webkitRequestFullscreen) {
    el.webkitRequestFullscreen().then(function() {
      // Votre code ici
    }).catch(function() {
      // Votre code ici
    });;
  } else if (el.msRequestFullscreen) {
    el.msRequestFullscreen().then(function() {
      // Votre code ici
    }).catch(function() {
      // Votre code ici
    });;
...
function volumeChangeIcon(e) {
  if (e.classList.contains('active')) {
    volume_muted.forEach(item=>{
      item.style.display = "none";
    })
    volume_high.forEach(item=>{
...
function drawProgress(canvas, buffered, duration) {
  ...
  if (!height || !width) throw "La largeur ou la hauteur du canvas n'est pas définie.";
...
function toggleFullScreenMode() {
  if (document.fullscreenElement == null) {
    removeSettings()
    video_player.requestFullscreen().then(function() {
      let portrait = false
      video.oncanplay = ()=>{
