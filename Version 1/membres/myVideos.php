...
<nav>
    ...
    <div class="profileBx">
        <a href="../profil/<?= $_GET['vs'] ?>"><img class="profileImg" src="<?= $avatar ?>" alt=""></a>
        <?php
        if (isset($_GET['vs']) && isset($_SESSION['id']) && ($_GET['vs'] != $_SESSION['id']) || !isset($_SESSION['id'])) {
        ?>
        <h3>Videos de <a href="../profil/<?= $_GET['vs'] ?>">
                <?= $pseudo ?>
            </a></h3>
        <?php
        } else {
    ...
    <div class="container">
        ...
        if (isset($_GET['vs']) && !empty($_GET['vs'])) {
            ...
            $fetch_video->execute(array($_GET['vs']));
        } else {
            $fetch_video = $bdd->prepare(' SELECT id, titre, poster, vues, date FROM video WHERE id_auteur = ? ORDER BY date DESC');
        ...
        if ($fetch_video->rowcount() == 0) {
            if(isset($_SESSION['auth']) && $_SESSION['id'] === $_GET['vs']){
                ?>
                <div class="empty">
                    Aucune vidéo :| <br>
                    Cliquez <a href="../post">ici</a> pour publier une vidéo ;)
                </div>
                <?php
            }else{
                ?>
                ...
                <div class="asideVid">
                    ...
                    <div class="vidInfos">
                        <div class="top">
                            ...
                            <div class="date">
                                <a href="../profil/<?= $_GET['vs'] ?>"> <?= $pseudo ?> </a>
                        ...
                        if (isset($_GET['vs']) && isset($_SESSION['id']) && ($_GET['vs'] != $_SESSION['id']) || !isset($_SESSION['id'])) {
                                    ?>
                        <div class="bottom">
                            <img onclick="location.replace('../profil/<?= $_GET['vs'] ?>')" src="<?= $avatar ?>"> <a href="../profil/<?= $_GET['vs'] ?>">
