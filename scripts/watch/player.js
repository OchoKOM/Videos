...
sendBtn = document.querySelector('.current-time button');
...
<div class="ochovid" title="Voir sur OchoVid" data-link="time" data-url="time">
    <a href="./watch?v=${videoId}" target="_blank"></a>
</div>
<div class="counter">
  <div class="count">
...
<div class="controls">
    ...
    <div class="controls-list">
        <div class="controls-left">
            <button title="Lire (k)" class="play_pause icon actions">
            ...
            <button title="Suivant (n)" class="play-next-icon icon actions">
              <svg class="next-icon" viewBox="0 0 48 48">
                <path fill="currentColor" d="M34 36V12h3v24Zm-23 0V12l17.3 12Zm3-12Zm0 6.25L23.05 24 14 17.75Z"/>
              </svg>
            </button>
        ...
        <div class="controls-right">
            <div class="fa auto-play active actions" title="Lecture automatique activée"></div>
...
<div class="wrapper">
  <ul class="menu-bar">
    ...
    <li onclick="seeMore()">
      ...
      + des vidéos de ${document.querySelector('.usrName').innerText}
    </li>
  </ul>
  ...
  <div class="drop" id="quality-drop">
    ...
    <li data-quality="" class="active">
      <div class="icon check"></div>
        ${document.getElementById('main-video').getAttribute('sizes')}p
      <p></p>
    </li>
  </ul>
  </div>
...
function timepassed() {
  let times = document.querySelectorAll('.time');
  times.forEach(time => {
    let date1 = time.getAttribute('data-time');
    let date = new Date().getTime() / 1000;
    let difference = date - date1;
    time.innerHTML = `• Il y a  ${formatT(difference)}`
  });
  requestAnimationFrame(timepassed);
}
timepassed();

var imglods = document.querySelectorAll('[data-load]')
var links = document.querySelectorAll('.link')

imglods.forEach(imglod => {
    imglod.addEventListener('load', (event) => {

        document.getElementById("lnkVD" + imglod.getAttribute('data-uniqid')).classList.add('loaded')
    })
    imglod.src = imglod.getAttribute('data-load');
