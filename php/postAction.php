...
if (!isset($title)) {
    $msg = "Erreur de configuration serveur";
                ...
                if ($_FILES['file']['size'] <= $maxsize) {
                    if (move_uploaded_file($_FILES['file']['tmp_name'], "../".$target_file) && move_uploaded_file($_FILES['poster']['tmp_name'], "../".$target_poster)) {
                        ...
                        if($sql->execute(array($id, $title, $target_file, $duration, $quality, $target_poster, $_SESSION['id'], $date))){
                            $msg = array("success", $id);;
                        }else{
                            $msg = "Erreur au serveur";
                        }


                    }else {
                        $msg = "Erreur lors de l'impotation des fichiers vérifiez la taille maximale de votre serveur ou votre logiciel serveur";
                    }
                } else {
